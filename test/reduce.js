...
describe('Immutable: reduce', function() {
  var add = function(a, b) {return a + b;};
  var mult = function(a, b) {return a * b;};

  it('folds simple functions over arrays with the supplied accumulator', function() {
    eq(R.reduce(add, 0, I.List([1, 2, 3, 4])), 10);
    eq(R.reduce(mult, 1, I.List([1, 2, 3, 4])), 24);
  });

  it('dispatches to objects that implement `reduce`', function() {
    var obj = {x: I.List([1, 2, 3]), reduce: function() { return 'override'; }};
    eq(R.reduce(add, 0, obj), 'override');
    eq(R.reduce(add, 10, obj), 'override');
  });

  it('returns the accumulator for an empty array', function() {
    eq(R.reduce(add, 0, I.List()), 0);
    eq(R.reduce(mult, 1, I.List()), 1);
    eq(R.reduce(R.concat, I.List(), I.List()), I.List());
  });
  ...
  it('is curried', function() {
    ...
    var sum = addOrConcat(0);
    var cat = addOrConcat('');
    eq(sum(I.List([1, 2, 3, 4])), 10);
    eq(cat(I.List(['1', '2', '3', '4'])), '1234');
  });
  ...
  it('Prefers the use of the iterator of an object over reduce (and handles short-circuits)', function() {
    ...
    Reducible.prototype.reduce = function(f, init) {
      var acc = init;
      for (var i = 0; i < this.arr.size; i += 1) {
        acc = f(acc, this.arr.get(i));
      }
      return acc;
    };
    ...
    var rfn = xf(apendingT);
    var list = new Reducible(I.List([1, 2, 3, 4, 5, 6]));

    eq(R.reduce(rfn, I.List(), list), I.List([1, 2]));
  });
});
