...
describe('Immutable: chain', function() {
  ...
  it('maps a function over a nested list and returns the (shallow) flattened result', function() {
    eq(R.chain(times2, I.List([1, 2, 3, 1, 0, 10, -3, 5, 7])), I.List([2, 4, 6, 2, 0, 20, -6, 10, 14]));
    eq(R.chain(times2, I.List([1, 2, 3])), I.List([2, 4, 6]));
  });
  ...
  it('does not flatten recursively', function() {
    ...
    eq(R.chain(f, I.List([[1], [[2], 100], [], [3, [4]]])), I.List([1, [2], 3]));
  });

  it('maps a function (a -> [b]) into a (shallow) flat result', function() {
    eq(intoArray(R.chain(times2), I.List([1, 2, 3, 4])), I.List([2, 4, 6, 8]));
  });
  ...
  it('interprets ((->) r) as a monad', function() {
    ...
    eq(R.chain(R.append, R.head)(I.List([1, 2, 3])), I.List([1, 2, 3, 1]));
  });

  it('dispatches to objects that implement `chain`', function() {
    var obj = I.Map({x: 100, chain: function(f) { return f(this.get('x')); }});
    eq(R.chain(add1, obj), [101]);
  });
  ...
  it('can compose transducer-style', function() {
    ...
    var mdec = R.chain(dec);
    var xcomp = R.compose(mdec, mdouble);
    eq(intoArray(xcomp, I.List([10, 20, 30])), I.List([18, 38, 58]));
  });

  it('is curried', function() {
    var flatInc = R.chain(add1);
    eq(flatInc(I.List([1, 2, 3, 4, 5, 6])), I.List([2, 3, 4, 5, 6, 7]));
  });
