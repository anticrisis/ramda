...
describe('Immutable: assocPath', function() {
  it('makes a shallow clone of an object, overriding only what is necessary for the path', function() {
    var obj1 = I.Map({a: I.Map({b: 1, c: 2, d: {e: 3}}), f: I.Map({g: I.Map({h: 4, i: I.List([5, 6, 7]), j: I.Map({k: 6, l: 7})})}), m: 8});
    var obj2 = R.assocPath(['f', 'g', 'i', 1], 42, obj1);
    eq(obj2.getIn(['f', 'g', 'i']), I.List([5, 42, 7]));
    // Note: reference equality below!
    assert.strictEqual(obj2.get('a'), obj1.get('a'));
    assert.strictEqual(obj2.get('m'), obj1.get('m'));
    assert.strictEqual(obj2.getIn(['f', 'g', 'h']), obj1.getIn(['f', 'g', 'h']));
    assert.strictEqual(obj2.getIn(['f', 'g', 'j']), obj1.getIn(['f', 'g', 'j']));
  });

  it('is the equivalent of clone and setPath if the property is not on the original', function() {
    var obj1 = I.Map({a: 1, b: {c: 2, d: 3}, e: 4, f: 5});
    var obj2 = assocPath(['x', 0, 'y'], 42, obj1);
    eq(obj2, I.Map({a: 1, b: {c: 2, d: 3}, e: 4, f: 5, x: [{y: 42}]}));
    // Note: reference equality below!
    assert.strictEqual(obj2.get('a'), obj1.get('a'));
    assert.strictEqual(obj2.get('b'), obj1.get('b'));
    assert.strictEqual(obj2.get('e'), obj1.get('e'));
    assert.strictEqual(obj2.get('f'), obj1.get('f'));
  });

  it('is curried', function() {
    var obj1 = I.Map({a: {b: 1, c: 2, d: {e: 3}}, f: {g: {h: 4, i: 5, j: {k: 6, l: 7}}}, m: 8});
    var expected = I.Map({a: {b: 1, c: 2, d: {e: 3}}, f: {g: {h: 4, i: {x: 42}, j: {k: 6, l: 7}}}, m: 8});
    var f = R.assocPath(['f', 'g', 'i']);
    var g = f({x: 42});
    eq(f({x: 42}, obj1), expected);
    eq(g(obj1), expected);
  ...
  it('empty path replaces the the whole object', function() {
    eq(R.assocPath([], 3, I.Map({a: 1, b: 2})), 3);
  });

  it('replaces `undefined` with a new object', function() {
    eq(assocPath(['foo', 'bar', 'baz'], 42, I.Map({foo: undefined})), I.Map({foo: {bar: {baz: 42}}}));
  });

  it('replaces `null` with a new object', function() {
    eq(assocPath(['foo', 'bar', 'baz'], 42, I.Map({foo: null})), I.Map({foo: {bar: {baz: 42}}}));
  });
